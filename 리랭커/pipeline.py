...
class QueryPipelineImpl:
    def __init__(self, retriever, generator, reranker=None, top_k: int = 40):
        self._retriever = retriever
        self._generator = generator
        self._reranker = reranker      # None이면 리랭킹 생략(검색 결과 그대로)
        self._top_k = top_k
    ...
    async def answer(self, query: str, auth: AuthContext,
        ...
        # 3) 생성(안전 가드는 생성기 내부 — 빈/저점수면 abstain, 인용 검증)
        gen = self._generator.generate(query=query, context=results)

        # 4) AnswerResult로 변환. 인용된 청크만 컨텍스트로 노출(출처 표시용).
