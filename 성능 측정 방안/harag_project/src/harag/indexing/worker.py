...
class IndexingWorker:
    ...
    def __init__(self, decoder: HwpDecoder, model: EmbeddingModel, morph: Morph,
                 ...
                 incremental: bool = False, object_store=None, metadata_store=None):
        ...
        self._metadata_store = metadata_store # None이면 메타 기록 생략
    ...
    def process(self, job: IngestJob) -> IngestResult:
        ...
        if ir.parse_status == ParseStatus.failed:
            return self._quarantine(job, "parse_failed")
        ...
        if has_table and ir.parse_quality.table_recovery_avg < self._min_table_recovery:
            return self._quarantine(job, f"low_table_recovery:{ir.parse_quality.table_recovery_avg:.2f}")
        ...
        if not chunks:
            return self._quarantine(job, "no_chunks")
        ...
        # 운영 메타 기록(PG가 진실 원천) — Qdrant 활성 포인터와 일관
        if self._metadata_store is not None:
            self._metadata_store.register_document(
                job.document_id, job.filename, job.department, "")
            self._metadata_store.record_version(
                job.document_id, version, len(chunks),
                ir.parse_quality.table_recovery_avg)
            self._metadata_store.set_active_version(job.document_id, version)
    ...
    def _quarantine(self, job, reason: str) -> "IngestResult":
        """격리 처리 — PG에 상태 기록(있으면) 후 결과 반환."""
        if self._metadata_store is not None:
            self._metadata_store.register_document(
                job.document_id, job.filename, job.department, "")
            self._metadata_store.update_status(job.document_id, "quarantined", reason)
        return IngestResult(job.document_id, IngestStatus.quarantined, reason=reason)
